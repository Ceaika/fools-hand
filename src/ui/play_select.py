...
_BLACK      = (10,  10,  10)
_TEAL       = (60,  210, 180)
_TEAL_DARK  = (20,  110,  90)


class PlaySelectScreen:
    """
    Mode selection. Returns: 'singleplayer' | 'tutorial' | 'back' | 'quit'
    """
    ...
    def __init__(self, screen: pygame.Surface, fonts: dict,
        ...
        self._tut_rect = pygame.Rect(right_x, card_y, card_w, card_h)
    ...
    def handle_event_with_rect(self, event: pygame.event.Event) -> tuple:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self._tut_rect.collidepoint(event.pos):
                audio.play("menu_click")
                return "tutorial", self._tut_rect.copy()
        return None, None
    ...
    def _draw_sp_card(self, target: pygame.Surface, f: dict) -> None:
        ...
        title = f["btn"].render(_t("play_select.duel"), False,
                                 NEON_GLOW if hover else TEXT_MAIN)
        target.blit(title, (r.centerx - title.get_width() // 2,
                             r.y + r.h // 2 + 14))
        sub = f["small"].render(_t("play_select.duel_sub"), False, TEXT_DIM)
        target.blit(sub, (r.centerx - sub.get_width() // 2,
                           r.y + r.h // 2 + 14 + title.get_height() + 10))

    def _draw_mp_card(self, target: pygame.Surface, f: dict) -> None:
        r = self._tut_rect
        ...
        title = f["btn"].render(_t("play_select.multi_bot"), False, TEXT_DIM)
        target.blit(title, (r.centerx - title.get_width() // 2,
                             r.y + r.h // 2 + 14))
        sub = f["small"].render(_t("play_select.multi_sub"), False, TEXT_DIM)
        target.blit(sub, (r.centerx - sub.get_width() // 2,
                           r.y + r.h // 2 + 14 + title.get_height() + 10))

        self._draw_road_sign(target, r, f)

    def _draw_road_sign(self, target: pygame.Surface, card: pygame.Rect, f: dict) -> None:
        import math
        pulse    = abs(math.sin(self.tick * 0.06)) * 0.4 + 0.6
        amber    = tuple(int(c * pulse) for c in _AMBER)
        wip      = f["btn"].render(_t("play_select.wip"), False, amber)
    ...
    def _draw_header(self, target: pygame.Surface, f: dict) -> None:
        pulse = abs(math.sin(self.tick * 0.03)) * 4
        title = f["title"].render(_t("play_select.title"), False, TEXT_MAIN)
